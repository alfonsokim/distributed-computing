...
public class ChatClient extends UnicastRemoteObject implements IChatClient {
	...
	private void connect() {
		try {
			server = (IChatServer) Naming.lookup("rmi://" + serverURL
					+ "/ChatServer");
	...
	public void receiveEnter(String name) {
		/* Aqui comente este mensaje en la consola, ya que se
		 * repite con el mensaje impreso en main() para pedir
		 * la cadena.
		 */
		// System.out.println("\nLog in " + name + "\n" + this.name
		// 		+ " -- Cadena a enviar: ");
	...
	public void receiveMessage(Message message) {
		/* Lo mismo, este mensaje se repite con el del main()
		 */
		// System.out.println('\n' + message.name + ":\n" + message.text + "\n"
	...
	public static String pideCadena(String letrero) {
		...
		try {
			...
			for (int i = 0; bArray[i] != 13 && bArray[i] != 10
					&& bArray[i] != 0; i++) {
